...
EXPECTED_SOURCE_COMMIT = "8147c3cbe46c128e3cd85833a3c3d042c9d9bc47"
...
def _completed_batch_candidates(results_root: Path) -> list[dict[str, Any]]:
    ...
    for path in batch_root.glob("*/batch-result.json"):
        ...
        if plan.get("strategy_sha256") != base.STRATEGY_SHA256:
            continue
        if plan.get("source_commit") != EXPECTED_SOURCE_COMMIT:
            continue
...
def select_locked_batch(results_root: Path) -> dict[str, Any]:
    ...
    if not candidates:
        raise RuntimeError(
            "No complete 10-coin diagnostic batch from the locked source commit was found. "
            "The causal analysis refuses to mix standalone runs from different batches."
        )
    if len(candidates) > 1:
        candidates.sort(
            key=lambda item: str(item["state"].get("finished_at_utc") or ""),
            reverse=True,
        )
    ...
    with tempfile.TemporaryDirectory(prefix="hixton-cohort-") as tmp:
        ...
        for pair, run_id in run_ids.items():
            ...
            if not source.is_dir():
                raise RuntimeError(
                    f"Locked batch {batch['batch_id']}: missing run folder {run_id} for {pair}"
                )
            target = view / run_id
            try:
