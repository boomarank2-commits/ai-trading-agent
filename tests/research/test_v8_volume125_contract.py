...
def test_b2_changes_exactly_the_preregistered_entry_dimension() -> None:
    text = STRATEGY.read_text(encoding="utf-8")
    constants = _class_constants()

    assert constants["RESEARCH_BACKTEST_ONLY"] is True
    assert constants["VOLUME_RATIO_MIN"] == 1.25
    assert '(dataframe["volume_ratio"] >= self.VOLUME_RATIO_MIN)' in text
    assert "slow_20d_donchian_breakout_vr125" in text
...
def test_b2_cannot_place_dryrun_or_live_entries() -> None:
    text = STRATEGY.read_text(encoding="utf-8")
...
def test_b2_does_not_mutate_the_v8_execution_config() -> None:
    # Research B2 is a one-variable strategy experiment. Configuration remains
