...
def simulate_variant(x: pd.DataFrame, v: Variant, mask: pd.Series, start_i: int, end_i: int, situations: pd.Series, fee: float = FEE) -> list[Trade]:
    ...
    high, low, close, mid = x["high"].to_numpy(), x["low"].to_numpy(), x["close"].to_numpy(), x["bb_mid"].to_numpy()
    out = []
    i = max(start_i, 1)
    while i < end_i - 1:
...
def train_router(x: pd.DataFrame, train_start_i: int, train_end_i: int, situations: pd.Series) -> tuple[dict, dict]:
    ...
    for v in candidates:
        ...
        for sit, ts in by_sit.items():
            m, stress = metrics(ts), metrics(ts, STRESS_FEE)
            if m["trades"] < 6 or m["pnl"] <= 0 or m["pf"] < 1.15 or stress["pnl"] <= 0:
                continue
...
def run_pair(symbol: str, df: pd.DataFrame) -> dict:
    start = df["date"].min()
    train_end = start + pd.Timedelta(days=TRAIN_DAYS)
    blind_end = train_end + pd.Timedelta(days=BLIND_DAYS)
    if df["date"].max() < blind_end - pd.Timedelta(minutes=15):
        raise RuntimeError(f"{symbol}: insufficient 1095-day data")
    x, q = features(df[df["date"] < blind_end].copy(), train_end)
...
def main() -> int:
    ...
    end = start + pd.Timedelta(days=TRAIN_DAYS + BLIND_DAYS)
    results = {}
    for symbol in PAIRS:
        print(f"=== {symbol}: download/load ===", flush=True)
        df = download_pair(symbol, start, end, args.cache)
        print(f"{symbol}: {len(df)} candles {df.date.min()} -> {df.date.max()}", flush=True)
        results[symbol] = run_pair(symbol, df)
        b = results[symbol]["blind"]
        print(f"{symbol}: blind trades={b['trades']} pnl={b['pnl']:.2f} PF={b['pf']:.3f} DD={b['dd']:.2f}", flush=True)
    payload = {"version": "V12_RESEARCH_SEARCH_1", "generated_at": datetime.now(UTC).isoformat(), "fee_per_side": FEE, "stress_fee_per_side": STRESS_FEE, "stake_usdt": STAKE, "train_days": TRAIN_DAYS, "blind_days": BLIND_DAYS, "results": results}
